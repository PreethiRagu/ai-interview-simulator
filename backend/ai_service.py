...
# Initialize Groq client only when a key exists.
...
def generate_insight(text: str):
    """Extracts skills and projects using LLM."""
    if client is None:
        return text[:1200]
...
def generate_questions_llm(analysis_or_config, count=5, interview_type="Technical"):
    """Generates interview questions based on analysis or config."""
    prompt = (
        f"Generate exactly {count} {interview_type} interview questions based on: {analysis_or_config}. "
    ...
    with get_db() as conn:
        conn.execute(
            ...
            (
                ...
                json.dumps(data.improvementRoadmap),
                data.feedbackSummary,
                json.dumps(data.resources),
                json.dumps(data.chartData),
                json.dumps(data.questionFeedback),
                created_at,
            ),
        )
...
async def generate_questions(config: InterviewConfig):
    print(f"DEBUG: Generating questions for: {config.role} in {config.domain}")
    try:
        if client is None:
        ...
        return {"questions": cleaned_questions[:config.numQuestions]}
    except Exception as e:
        print(f"CRITICAL ERROR in /generate-questions: {str(e)}") # This shows the truth
        return {"questions": build_local_questions(config)}
...
async def run_code(data: dict):
    # In a real scenario, use an external API like Piston/Judge0 here
    user_code = data.get("code", "")
    return {
        "output": f"Compiled successfully!\nResult for {len(user_code)} chars of code.",
        "status": "passed"
    }
...
async def get_dashboard_stats(userEmail: str | None = None):
    ...
    return {
        ...
        "performanceTrends": [{"name": f"Session {index + 1}", "score": row["score"]} for index, row in enumerate(recent_rows)],
